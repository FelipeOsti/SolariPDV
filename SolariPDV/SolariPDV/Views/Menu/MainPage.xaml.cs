            ...
            if (!MenuPages.ContainsKey(id))
            {
                switch (id)
                {
                    ...
                    case (int)MenuItemType.PedidoPDV:
                        MenuPages.Add(id, new NavigationGradient(new VerPedidosPage(Pedido.TipoPedido.PedidoPDV)) { BarTextColor = Color.White });
                        break;
                    case (int)MenuItemType.PedidoComercial:
                        MenuPages.Add(id, new NavigationGradient(new VerPedidosPage(Pedido.TipoPedido.PedidoComercial)) { BarTextColor = Color.White });
                        break;
